...
class TeacherControllerTest {
    ...
    private final Teacher randomTeacher = new Teacher(1L, 1L, "Donald", "Trump", 60);
    private final Department randomDepartment = new Department(1L, "Politic");
    private final List<Teacher> randomTeachers = Arrays.asList(new Teacher(1L, 3L, "Robert", "Kiyosaki", 58), new Teacher(2L, 6L, "Djordan", "Belfort", 67));
    ...
    void index_shouldReturnTeachersIndexView() throws Exception {
        ...
        when(departmentService.getById(randomTeachers.get(0).getDepartmentId())).thenReturn(randomDepartment);
        when(departmentService.getById(randomTeachers.get(1).getDepartmentId())).thenReturn(randomDepartment);
